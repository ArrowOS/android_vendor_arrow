...
function setup_vendor() {
    ...
    export ANDROIDBP="$ARROW_ROOT"/"$OUTDIR"/Android.bp
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        ...
        printf " * Copyright (C) 2019-$YEAR ArrowOS\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR ArrowOS\n" >> $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ARROW_ROOT"/prebuilts/tools-extras/common/smali/baksmali.jar
        export SMALIJAR="$ARROW_ROOT"/prebuilts/tools-extras/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ARROW_ROOT"/prebuilts/tools-extras/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ARROW_ROOT"/prebuilts/tools-extras/${HOST}-x86/bin/compact_dex_converter
    fi

    if [ -z "$PATCHELF" ]; then
        export PATCHELF="$ARROW_ROOT"/prebuilts/tools-extras/${HOST}-x86/bin/patchelf
    fi
...
function extract() {
    ...
    local OUTPUT_ROOT="$ARROW_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$ARROW_ROOT"/vendor/arrow/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
